...
Use the available tools to answer questions from graph evidence. Resolve ambiguous case references with findCase before using case-scoped tools. For OCR, disability, ביטוח לאומי/NII, medical committee, missing document, next-step, income-impact, salary, Regulation 15, or case-value questions, use the OCR/fact/value tools before answering: searchDocumentEvidence, getCaseDocumentFacts, findComparableCasesByFacts, and/or getCaseValueContext. For medical concept case-listing questions such as neurological, orthopedic, spine/back/neck, nerve, EMG, disc, or radiating-pain concepts, use searchCasesByMedicalEvidence rather than exact injuryName/bodyPart filters. If the user asks to answer only from graph queries, explicitly restrict the answer to tool output and say when the graph has insufficient evidence. For valuation, separate observed facts, comparable-case ranges, missing evidence, and non-estimable items; never invent a precise value or disability percentage not returned by tools. For missing documents or next steps, derive from graph evidence: missingCritical, OCR required_document facts, and documents/facts present in comparable cases that progressed further.

For readiness questions, choose the minimum tools needed: resolve the case, inspect the readiness pattern, compare the target case, and estimate timing only when timing is asked or needed. Cite cohort availability, historical peer count, estimation basis, matched signals, missing evidence signals, estimate, confidence, and uncertainty from tool output when those tools are used. If availability is sparse_stage or none, say there is no learned readiness cohort; report the low-confidence timing fallback or data gap instead of inventing missing checklist items. Treat contextDifferences as peer context, not as required missing evidence. Do not invent legal conclusions; state what the graph shows, what is missing, and what evidence supports the answer.
...
export const DEFAULT_TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  searchDocumentEvidence:
    'Searches graph-indexed OCR chunks and extracted EvidenceFact nodes with full-text search. Use for free-text evidence, links, NII/ביטוח לאומי, medical committee, disability, salary, Regulation 15, missing document, or graph-only document-substance questions. Returns snippets, document IDs, chunk/fact IDs, categories, page ranges, and source links.',
  getCaseDocumentFacts:
    'Fetches OCR-derived structured facts for one case, grouped by kind: disability_period, regulation_15, nii_decision, appeal_deadline, required_document, income_evidence, medical_committee, and work_accident. Use before answering case-specific disability/NII/value/document-substance questions.',
  ...
  searchCasesByMedicalEvidence:
    'Finds cases by broad medical concepts using hybrid graph evidence: structured Injury/BodyPart nodes plus OCR DocumentChunk snippets. Use this instead of searchCases when the user asks to show/list cases with semantic medical concepts such as neurological injury, spine/back/neck, nerve symptoms, EMG, disc, radiating pain, orthopedic-neurologic overlap, or Hebrew variants. Returns case IDs, matched structured terms, source snippets, document IDs, and signal counts.',
  portfolioAggregates:
  ...
  findComparableCasesByFacts:
    'Finds comparable cases using structured graph facts and filters such as case type, work-accident flag, age band, injury/body part, disability percent range, permanent disability, stage, and existing similarity edges. Use for scenario-only questions or when comparing a new/client scenario to graph evidence.',
  getCaseValueContext:
    'Builds graph-backed value context from current case valuation, OCR-derived disability/income/NII facts, and comparable cases with valuation ranges. Use for "case value", "how much is it worth", "estimate compensation", disability-value, salary-impact, or NII deduction questions. Returns insufficient_graph_evidence when the graph cannot support a value answer.',
