import { z } from 'zod';
import { dynamicEnumOptional } from '@/tools/_shared/dynamicEnums';
...
/**
 * Factory rather than a const because closed-enum fields (caseType, legalStage, phase, status)
...
export function buildInputSchema() {
  return z.object({
    caseType: dynamicEnumOptional(
      'caseType',
      'Exact match on c.caseType. OMIT unless the user asks about a specific type.'
    ),
    ...
    phase: dynamicEnumOptional(
      'phase',
      'Exact match on c.phase. OMIT unless the user asks about a specific phase.'
    ),
    status: dynamicEnumOptional(
      'status',
      "Exact match on c.status. OMIT unless the user explicitly asks about a specific status; do not default to 'open'."
    ),
    isSigned: z
      .boolean()
      .optional()
    ...
    mainInjury: z
      .string()
      .optional()
      .describe('Substring match (case-insensitive, Hebrew-normalized) on c.mainInjury.'),
    clientName: z
      .string()
      .optional()
      .describe('Substring match (case-insensitive) on the related Contact name via HAS_CLIENT.'),
    injuryName: z
      .string()
      .optional()
      .describe(
    ...
    hasDocumentCategory: z
      .string()
      .optional()
      .describe(
        "Cases that have at least one document of the given DocumentCategory name (e.g. 'medical_records')."
      ),
    missingDocumentCategory: z
      .string()
      .optional()
      .describe('Cases whose missingCritical array contains the given category name.'),
    completionRateMin: z
      .number()
      .min(0)
      .max(1)
      .optional()
      .describe('Lower bound (inclusive) for c.completionRate, 0-1.'),
    completionRateMax: z
      .number()
      .min(0)
      .max(1)
      .optional()
      .describe('Upper bound (inclusive) for c.completionRate, 0-1.'),
    monthsSinceEventMin: z
      .number()
      .optional()
      .describe('Lower bound (inclusive) for c.monthsSinceEvent.'),
    monthsSinceEventMax: z
      .number()
      .optional()
      .describe('Upper bound (inclusive) for c.monthsSinceEvent.'),
    monthsToSoLMax: z
      .number()
      .optional()
      .describe(
    ...
    eventDateFrom: z
      .string()
      .optional()
      .describe('Lower bound (inclusive) for c.eventDate as ISO string.'),
    eventDateTo: z
      .string()
      .optional()
      .describe('Upper bound (inclusive) for c.eventDate as ISO string.'),
    createdAtFrom: z
      .string()
      .optional()
      .describe('Lower bound (inclusive) for c.createdAt as ISO string.'),
    createdAtTo: z
      .string()
      .optional()
      .describe('Upper bound (inclusive) for c.createdAt as ISO string.'),
    signedAtFrom: z
      .string()
      .optional()
      .describe('Lower bound (inclusive) for c.signedAt as ISO string.'),
    signedAtTo: z
      .string()
      .optional()
      .describe('Upper bound (inclusive) for c.signedAt as ISO string.'),
...
export type SearchCasesInput = z.infer<SearchCasesInputSchema>;
